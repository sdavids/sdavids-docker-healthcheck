...
>|4.0K
|the `node` binary would add 41.1M
...
5.2M    /usr/local/bin/healthcheck
----
...
[NOTE]
====
This healthcheck is only useful in a Node.js Docker image, i.e. an image where the `node` binary already exists--the `node` binary would add 41.1M.
====
