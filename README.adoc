...
[,console]
----
$ docker run --platform=linux/amd64 --rm debian:13.2-slim sh -c "apt-get -qq update && apt-get -qq install -y wget=1.25.0-2 >/dev/null 2>&1 && ldd /usr/bin/wget | awk '{ print $ 3}' | xargs du -ckshL /usr/bin/wget"
472K   /usr/bin/wget
