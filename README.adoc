...
4.0M    /usr/local/bin/healthcheck
----
...
[NOTE]
====
This health check is only useful in a Node.js Docker image, i.e., an image where the `node` binary already exists--the `node` binary would add 41.2M.
====
